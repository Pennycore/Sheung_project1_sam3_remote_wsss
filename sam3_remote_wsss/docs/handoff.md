...
- Top-K metadata 频率诊断完成：OpenAI CLIP/RemoteCLIP 选择中 Manual4 提示的总体占比约为 `37.45%/27.78%`。RemoteCLIP 对全部197个 building patch 和238个 impervious surface patch 分别固定选择完全相同的四条提示，low vegetation/tree 的前三条也对所有 patch 不变；其选择主要是 `a remote sensing image of`、`in the center`、`there are several` 等B2C句式。说明排序主要受预训练句式偏置支配，而非图像内容或掩码效用，硬 Top-K 分支到此终止。
...
- 像素级评估协议已统一补充 `mF1` 和 `OA`：伪标签、student validation、patch test 与 stitched test 均报告 `mIoU/mF1/OA`、前景宏平均及逐类 IoU/F1；`pixel_accuracy` 保留为 `OA` 的兼容别名。带 ignore 的伪标签同时报告 strict、labeled-only 和 coverage。候选实例诊断仍使用 purity/precision/recall，只有重建完整伪标签后才计算 mF1/OA/mIoU。
