...
CAM 区域复核的 `top20` 结果已完成。全部候选与多阳性 patch 的结果几乎一致，说明结论不是
单阳性 patch 上“只能选一个类别”造成的虚高。全部候选中，CAM/SAM3 一致率为 `0.7064`；
一致过滤把候选主导类别正确率从 `0.8556` 提高到 `0.9262`，像素加权纯度从 `0.7796`
提高到 `0.8846`，错误候选拒绝率为 `0.6389`，但正确候选召回只有 `0.7647`。多阳性候选
对应指标为一致率 `0.7057`、过滤后主导正确率 `0.9259`、加权纯度 `0.8827`、正确召回
`0.7640`、错误拒绝率 `0.6389`。

逐类 top20 结果显示，impervious surface 的主导正确率由 `0.7289` 提高到 `0.8886`，正确
召回 `0.7778`、错误拒绝 `0.7378`；low vegetation 由 `0.7716` 提高到 `0.8773`，正确召回
`0.7989`、错误拒绝 `0.6226`。building 原始主导正确率已为 `1.0`，CAM 一致过滤仍会删除
约 `22.76%` 的正确候选；tree/car 的过滤后正确率虽提高到 `0.9725/0.9299`，但正确召回仅
`0.7794/0.7507`。因此全类别统一硬过滤不成立；当前证据只支持优先研究 impervious surface
和 low vegetation 的类别选择性拒绝。

`mean` CAM 的总体过滤后主导正确率/像素加权纯度/正确召回/错误拒绝率为
`0.9216/0.8745/0.7661/0.6139`；`top20` 对应为
`0.9262/0.8846/0.7647/0.6389`。总体上 top20 纯度略高，但在优先校正的两类中，mean 的
正确召回明显更高：impervious surface 为 `0.8617` 对 `0.7778`，low vegetation 为
`0.8771` 对 `0.7989`；过滤后主导正确率只分别低 `0.0110/0.0051`。由于当前方法的首要瓶颈
仍包括覆盖不足，下一组机制实验冻结为 mask 内 `mean CAM`，且只对 impervious surface 和
low vegetation 执行 CAM/SAM3 不一致拒绝。该选择仍是固定训练子集上的探索性规则，不作为
最终泛化结论；后续必须在父图互斥 validation 协议中确认。

新增 `rebuild_candidate_pseudo_labels`，可从候选缓存离线重建 baseline、全类别拒绝或指定类别
拒绝伪标签。CAM 只能拒绝、不能重分类；工具不读取 GT，输出每图和逐类 kept/rejected 统计，
并保护已有输出目录。下一步在固定256-patch上重建 baseline、all-class mean 和 selective mean
...
V2-a 逐父图稳定性通过：六张 validation 父图的 mIoU/mF1/OA 全部正增长。逐父图 mIoU
增量为 `+0.0665/+0.0123/+0.0362/+0.0107/+0.0206/+0.0231`；mF1 增量为
`+0.0766/+0.0110/+0.0469/+0.0143/+0.0239/+0.0278`；OA 增量为
`+0.0986/+0.0261/+0.0469/+0.0075/+0.0270/+0.0224`。父图宏平均 mIoU 从
`0.3406 +/- 0.0450` 提高到 `0.3688 +/- 0.0353`，mF1 从
`0.4471 +/- 0.0438` 提高到 `0.4805 +/- 0.0298`，OA 从
`0.3933 +/- 0.0839` 提高到 `0.4314 +/- 0.0629`。因此结果并非单一空间区域主导，V2-a
正式冻结为 dataset-specific mechanism ablation；后续不再搜索该 allowlist，转向区域语义
模型与 CAM 的通用一致性门控。
